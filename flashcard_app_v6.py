...
class FlashcardFletApp:
    def __init__(self, page: ft.Page):
        ...
        # Assets (paths are relative to the assets/ folder)
        ...
        # Audio
        # FIX 1: Audio objects belong in page.overlay, NOT page.services (page.services doesn't exist)
        ...
        # FIX: In Flet 0.80+, FilePicker added to page.overlay shows as a red
        # "Unknown control" box. Instead, assign on_result after creation and
        # add it to the page directly (it is non-visual and won't be seen).
    ...
    def build_panel(self, content: ft.Control, width: int = 430, height: int | None = None):
        return ft.Container(
            ...
            # FIX 3: ft.Border.all() (lowercase module) NOT ft.Border.all() (the class)
    ...
    # ---------- audio ----------
    # FIX 5: flet_audio uses stop(), not release() — release() doesn't exist
    async def _stop_music(self):
        if not self.audio_enabled:
            return
        try:
            await self.menu_music.stop()
        except Exception:
            pass
        try:
            await self.quiz_music.stop()
        except Exception:
            pass
        self.current_music = None
    ...
    # ---------- import ----------
    # FIX 2 (continued): Separate callback to handle FilePicker results
    ...
    # FIX 2 (continued): browse_txt_file is now a plain sync method — no async, no await
    ...
    def open_round_dialog(self):
        ...
        # FIX 6: page.dialog is deprecated — add AlertDialog to page.overlay instead
        dlg = ft.AlertDialog(
            modal=True,
        ...
        dlg.actions = [
            ft.TextButton("Cancel", on_click=cancel_round),
            ft.TextButton("Start", on_click=start_round),
        ]

        # Clean up any old dialogs from overlay before adding a new one
...
if __name__ == "__main__":
    # FIX 7: Use ft.run() — ft.app() is deprecated since Flet 0.80.0
    # Assets folder must be next to this script:
    #   assets/
    #     background_images/space-wallpaper-stars-background.gif
    #     sounds/menu_music.mp3, quiz_music.mp3, correct.mp3, wrong.mp3
