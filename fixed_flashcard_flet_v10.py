...
class FlashcardFletApp:
    def __init__(self, page: ft.Page):
        ...
        # Note: these two tracks are intentionally mapped this way because the observed playback
        # behavior showed the menu and quiz songs were effectively reversed in the project assets.
        self.background_src = "/background_images/space-wallpaper-stars-background.gif"
        self.menu_music_src = "/sounds/quiz_music.mp3"
        self.quiz_music_src = "/sounds/menu_music.mp3"
        ...
        self.menu_music_file = self.assets_dir / "sounds" / "quiz_music.mp3"
        self.quiz_music_file = self.assets_dir / "sounds" / "menu_music.mp3"
    ...
    def setup_audio(self):
        try:
            ...
            if self.asset_exists(self.menu_music_file):
                initial_music = self.menu_music_src
            elif self.asset_exists(self.quiz_music_file):
                initial_music = self.quiz_music_src

            self.bgm = fta.Audio(
                src=initial_music,
                volume=self.music_volume_for("menu"),
                autoplay=not getattr(self.page, "web", False),
                release_mode=fta.ReleaseMode.LOOP,
                on_loaded=lambda e: self.schedule_music_for_current_screen(force=True),
            )
    ...
    async def switch_music_once(self, target: str, force: bool = False) -> bool:
        ...
        if not force and self.current_music == target and getattr(self.bgm, "src", None) == wanted_src:
            return True

        try:
            src_changed = getattr(self.bgm, "src", None) != wanted_src
