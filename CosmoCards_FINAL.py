...
class FlashcardFletApp:
    def __init__(self, page: ft.Page):
        ...
        # Menu music can be named either main_menu.mp3 or menu_music.mp3.
        ...
        self.menu_music_candidates = [
            ("main_menu.mp3", "/sounds/main_menu.mp3"),
            ("menu_music.mp3", "/sounds/menu_music.mp3"),
        ]
        self.quiz_music_candidates = [
            ("quiz_music.mp3", "/sounds/quiz_music.mp3"),
        ]
        self.menu_music_file, self.menu_music_src = self.resolve_sound_asset(self.menu_music_candidates)
        self.quiz_music_file, self.quiz_music_src = self.resolve_sound_asset(self.quiz_music_candidates)
        self.correct_sound_file = self.assets_dir / "sounds" / "correct.mp3"
        self.wrong_sound_file = self.assets_dir / "sounds" / "wrong.mp3"

        # Page setup
        self.page.title = "CosmoCards"
    ...
    def resolve_sound_asset(self, candidates: list[tuple[str, str]]):
        sounds_dir = self.assets_dir / "sounds"
        for filename, asset_src in candidates:
            candidate_path = sounds_dir / filename
            if self.asset_exists(candidate_path):
                return candidate_path, asset_src
        filename, asset_src = candidates[0]
        return sounds_dir / filename, asset_src
    ...
    def music_target_for_screen(self) -> str | None:
        if self.current_screen == "quiz":
            return "quiz"
        if self.current_screen in ("menu", "manual", "final"):
            return "menu"
        return None
    ...
    def music_src_for(self, target: str):
        if target == "quiz":
            return self.quiz_music_src
        return self.menu_music_src
    ...
    def setup_audio(self):
        try:
            ...
            if self.asset_exists(self.menu_music_file):
                self.menu_bgm = fta.Audio(
                    src=self.menu_music_src,
                    volume=self.music_volume_for("menu"),
                    autoplay=False,
                    release_mode=fta.ReleaseMode.LOOP,
                )

            if self.asset_exists(self.quiz_music_file):
                self.quiz_bgm = fta.Audio(
                    src=self.quiz_music_src,
                    volume=self.music_volume_for("quiz"),
                    autoplay=False,
                    release_mode=fta.ReleaseMode.LOOP,
                )

            self.correct_sfx = fta.Audio(
                src=self.correct_sound_src,
                volume=0.9,
                autoplay=False,
                release_mode=fta.ReleaseMode.STOP,
            )
            self.wrong_sfx = fta.Audio(
                src=self.wrong_sound_src,
                volume=0.9,
                autoplay=False,
                release_mode=fta.ReleaseMode.STOP,
            )
            ...
            services = []
            if self.menu_bgm:
                services.append(self.menu_bgm)
            if self.quiz_bgm:
                services.append(self.quiz_bgm)
            services.extend([self.correct_sfx, self.wrong_sfx])

            try:
                self.page.services.extend(services)
                self.page.update()
            except Exception:
                pass
    ...
    # ---------- audio ----------
    def player_for(self, target: str):
        if target == "quiz":
            return self.quiz_bgm
        return self.menu_bgm

    def other_player_for(self, target: str):
        if target == "quiz":
            return self.menu_bgm
        return self.quiz_bgm

    async def stop_player(self, player):
        if not player:
            return
        try:
            await player.pause()
    ...
    async def delayed_startup_music(self):
        ...
        for delay in (0.0, 0.15, 0.5, 1.0):
            ...
            if self.current_screen not in ("menu", "manual", "final"):
                return
    ...
    async def switch_music_with_retry(self, target: str, force: bool = False):
        ...
        if not player or not self.asset_exists(music_file):
            return
    ...
    async def switch_music_once(self, target: str, force: bool = False) -> bool:
        target_player = self.player_for(target)
        other_player = self.other_player_for(target)
        ...
        try:
            target_player.volume = self.music_volume_for(target)
        except Exception:
            pass

        try:
            await self.stop_player(other_player)
        except Exception:
            pass
        ...
        try:
            await target_player.seek(0)
        except Exception:
            pass

        try:
            await target_player.play(0)
        except Exception:
            try:
                await target_player.play()
            except Exception:
                try:
                    await target_player.resume()
                except Exception:
                    return False
    ...
    def check_answer(self, selected_answer: str):
        ...
        self.render()
        if self.audio_enabled and self.current_screen == "final":
    ...
    def build_menu_screen(self):
        ...
        content = ft.Column(
            ...
            controls=[
                ft.Text("CosmoCards", size=30, weight=ft.FontWeight.BOLD, color=self.title_color),
                ft.Text(
