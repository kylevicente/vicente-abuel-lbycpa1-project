...
class FlashcardFletApp:
    def __init__(self, page: ft.Page):
        ...
        if self.audio_enabled:
            self.menu_music = fta.Audio(
                src=self.menu_music_src,
                volume=0.6,
                autoplay=False,
                release_mode=fta.ReleaseMode.STOP,
            )
            self.quiz_music = fta.Audio(
        ...
        if not self.audio_enabled:
            self.show_message("Sound is off. Install with: pip install flet-audio")
        else:
            self.page.run_task(self.start_menu_music)
    ...
    def build_panel(self, content: ft.Control, width: int = 430):
        return ft.Container(
            content=content,
            width=width,
            padding=20,
    ...
    def button_content(self, text, color):
        return ft.Container(
            content=ft.Text(
                text,
                color=color,
    ...
    def main_button(self, text, on_click):
        return ft.Button(
            content=self.button_content(text, "white"),
            on_click=on_click,
            width=250,
    ...
    def answer_button(self, text, on_click):
        return ft.Button(
            content=self.button_content(text, "black"),
            on_click=on_click,
            width=320,
            style=ft.ButtonStyle(
    ...
    async def start_menu_music(self):
        if not self.audio_enabled or self.current_music_name == "menu":
            return
        try:
        ...
        try:
            ...
            self.show_message(f"Menu music error: {error}")

    async def start_quiz_music(self):
        if not self.audio_enabled or self.current_music_name == "quiz":
            return
        try:
        ...
        try:
            ...
            self.show_message(f"Quiz music error: {error}")

    async def play_correct_sfx(self):
        if not self.audio_enabled:
            return
        try:
    ...
    # Import options
    # -------------------------
    async def pick_txt_file(self, e=None):
        """
        Uses the direct FilePicker pattern from current Flet docs.
        """
        try:
            files = await ft.FilePicker().pick_files(allow_multiple=False)
        except Exception as error:
            self.show_message(f"File picker not available here: {error}")
            return
        ...
        if not file_path.lower().endswith(".txt"):
            self.show_message("Please select a TXT file.")
            return

        self.load_flashcards_from_file(file_path)

    def load_from_path_field(self, e=None):
    ...
    def start_quiz(self, count: int):
        ...
        self.render()
        if self.audio_enabled:
            self.page.run_task(self.start_quiz_music)
    ...
    def check_answer(self, selected_answer: str):
        if selected_answer == self.correct_answer:
            ...
            if self.audio_enabled:
                self.page.run_task(self.play_correct_sfx)
        else:
            self.show_message(f"Wrong! Correct answer: {self.correct_answer}")
            if self.audio_enabled:
                self.page.run_task(self.play_wrong_sfx)
    ...
    def build_menu_screen(self):
        ...
        content = ft.Column(
            ...
            controls=[
                ...
                ft.Text(
                    "Import options: browse, paste a local path, or paste the TXT content itself.",
                    size=12,
                    italic=True,
                    color=self.subtle_text,
                    text_align=ft.TextAlign.CENTER,
                ),
                self.main_button("Browse TXT File", self.pick_txt_file),
                self.path_box,
                self.main_button("Load From Path", self.load_from_path_field),
                self.paste_box,
