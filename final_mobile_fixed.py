...
class FlashcardFletApp:
    ...
    def main_button(self, text, on_click, icon_name=None):
        row_controls = []
        if icon_name is not None:
            row_controls.append(ft.Icon(icon_name, color="white", size=18))
        row_controls.append(
            ft.Text(
                text,
                color="white",
    ...
    async def pick_txt_file(self, e):
        try:
            ...
            self.show_message(f"Could not open file picker: {error}")
            return
