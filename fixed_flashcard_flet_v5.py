...
class FlashcardFletApp:
    ...
    def schedule_music_for_current_screen(self):
        if not self.audio_enabled:
            return

        target = self.music_target_for_screen()
        if target:
            self.page.run_task(self.switch_music_with_retry, target)
    ...
    # ---------- audio ----------
    async def switch_music_with_retry(self, target: str):
        if not self.audio_enabled:
            return

        if target == "menu" and not self.asset_exists(self.menu_music_file):
            return
        if target == "quiz" and not self.asset_exists(self.quiz_music_file):
            return

        self.music_request_id += 1
        request_id = self.music_request_id

        delays = [0.0, 0.25, 0.75] if target == "menu" else [0.0, 0.15, 0.45]

        for delay in delays:
            if delay > 0:
                await asyncio.sleep(delay)

            if request_id != self.music_request_id:
                return

            ok = await self.switch_music_once(target)
            if ok:
                return
        ...
        if not self.audio_message_shown:
            ...
            self.show_message(
                "Audio did not start automatically. Tap any button once and it should begin."
            )

    async def switch_music_once(self, target: str) -> bool:
        if target == "menu":
            wanted = self.menu_music
            other = self.quiz_music
        else:
            wanted = self.quiz_music
            other = self.menu_music
    ...
    async def play_menu_music(self):
        await self.switch_music_with_retry("menu")

    async def play_quiz_music(self):
        await self.switch_music_with_retry("quiz")
